...
public class Handler : IRequestHandler<Query, Result<QueryResponse>>
{
    ...
    {
        var activeCount = await _dbContext.Assets.CountAsync(a => a.IsActive, cancellationToken);
        var inactiveCount = await _dbContext.Assets.CountAsync(a => !a.IsActive, cancellationToken);
