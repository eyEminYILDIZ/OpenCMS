...
type FormValues = Omit<DispatchApi.Update.Request, 'id'>;

export const DispatchUpdateSheet = observer(() => {
  const { dispatchStore } = stores;
  const { t } = useTranslation();

  const visible = dispatchStore.panelMode === PanelModes.Update;
  const selectedItem = dispatchStore.selectedItem;

  const buildInitialValues = (): FormValues => ({
    title: selectedItem?.title ?? '',
    description: selectedItem?.description ?? '',
    category: selectedItem?.category ?? DispatchApi.Enums.DispatchCategories.Operation,
    occuredAt: selectedItem?.occuredAt ?? '',
    relatedEntityId: selectedItem?.relatedEntityId ?? GuidService.generateEmptyGuid(),
    relatedChildEntityId: selectedItem?.relatedChildEntityId ?? null,
    providerAgentId: selectedItem?.providerAgentId ?? agentId,
  });
  ...
  const formik = useFormik<FormValues>({
    ...
    onSubmit: async (values) => {
      await dispatchStore.updateItem(values);
    },
  });
      ...
      <View style={styles.overlay}>
        ...
        >
          <View style={styles.header}>
            <Text style={styles.title}>{t('dispatch.update.title')}</Text>
